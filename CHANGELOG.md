...
MotionKernel's own history is in the `Unreleased (downstream)` and downstream
sections below. The `v1.3.0` and earlier sections are the **upstream AutoKernel
release history**, preserved as provenance. They are not MotionKernel releases.

## Unreleased (downstream)

### V1 release preparation

- Replaced the `motionkernel` import alias. It resolved through a
  `sys.meta_path` finder, which preserved module identity but was invisible to
  static analysis: `from motionkernel.specs import Tolerance` type-checked as
  `Any` while the `autokernel` spelling resolved. It is now plain re-export
  modules, so both namespaces type-check and class identity is shared
  (`isinstance` works across them). The trade is module identity and deep
  module paths, neither of which anything depended on.
- Added `docs/DELETION_INVENTORY.md`, generated by
  `scripts/legacy_reachability.py`, which checks six independent reachability
  signals per inherited file. It proposes **no** deletions: every candidate is
  still reachable, and for most of them the only signal is that `README.md`
  documents them as commands. Deletion is a scoping decision, not a code fact,
  and the report says so.
- Support matrix gains a **candidate** level. Cosmos is a candidate, not
  supported: integration work exists but no authoritative end-to-end evidence
  has been published.
- Added `docs/MIGRATION_V1.md` and a draft `docs/TAG_NOTES_DRAFT.md`.
- Release checklist gains repository-settings verification, including private
  vulnerability reporting and leaving the fork relationship intact.

### Public-release readiness

- Added `motionkernel` as the canonical import namespace. It aliases
  `autokernel` rather than duplicating it, so `motionkernel.specs is
  autokernel.specs` and class identity is shared across both names. The
  `autokernel` namespace is unchanged, un-deprecated, and still what generated
  `spec.py` files import, because artifact bundles are hash-verified and
  rewriting that import would invalidate every artifact already produced. The
  four-phase migration and the gate on each phase are documented in
  `docs/NAMESPACE_MIGRATION.md`.
- Fixed a release-identity drift: `pyproject.toml` declared version `0.1.0`
  while `autokernel.__version__` declared `1.0.0`. They are now one version and
  a test fails if they diverge again.
- Added `PROVENANCE.md`, generated by `scripts/provenance_inventory.py` from
  Git history against the upstream base commit, classifying every file as
  unchanged-upstream, modified-descendant, or MotionKernel-original. It can be
  re-derived and checked rather than trusted.
- Removed upstream's `v1.x` release notes from the README, where they appeared
  under a "Changelog" heading and read as MotionKernel's own release history.
- Added `docs/SUPPORT_STATUS.md`, which defines four support levels and admits
  a claim only with a linked evidence file. LTX2 is *proven* with its scope
  stated; Wan is *validated (isolated)* with no end-to-end claim; Cosmos is
  *in progress*; Kandinsky is a *target*.
- Removed an inherited, unsupported throughput claim ("~90 seconds per
  experiment, ~320 overnight") and relabelled the nine inherited built-in
  kernels as starter specifications rather than supported kernels.
- Added `docs/ARCHITECTURE.md`, `docs/REPRODUCIBILITY.md`,
  `docs/RELEASE_CHECKLIST.md`, and `SECURITY.md`. The security policy states
  the threat model explicitly, including that loading an artifact executes
  code and that hash verification establishes integrity, not trustworthiness.
- Added package metadata that was missing for a public release: license file,
  classifiers, keywords, and project URLs.
- Added `tests/test_public_metadata.py`, which fails on drift in product
  naming, package metadata, upstream attribution, relative documentation
  links, the compatibility namespace, and unevidenced support claims.
- Added Ruff configuration. Inherited upstream directories are excluded so lint
  findings in MotionKernel-original code are not buried in noise.
